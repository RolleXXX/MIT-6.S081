...
// 辅助函数，用于首先释放进程内核表的所有叶映射，
// 但不释放物理地址，然后删除所有3级间接和此内核页本身的物理地址
...
freewalk(pagetable_t pagetable)
{
  ...
  for(int i = 0; i < 512; i++){
    ...
    // PTE指向下一层而不是终末层的判定是pte & (PTE_R|PTE_W|PTE_X)
    // 因为在walk中间接层只有PTE_V被设立，而在终末层所有flag都被设立
...
// 所以这个函数只需要把在kpagetable里所有 间接 mapping清除即可
void
...
void vmprint_helper(pagetable_t pagetable,int depth){
  ...
  if(depth<=0||depth>=4){
    printf("vmprintf_helper: depth not in {1,2,3}\n");
  }
  // there are 2^9 = 512 PTEs in a page table.
  for (int i = 0; i < 512;i++){
    pte_t pte = pagetable[i];
